...
regex="ibm-semeru(-open|-certified)(-jre|-jdk)_([[:alnum:]\-]+)_([[:alnum:]]+).*\.(tar\.gz|zip|pkg|msi|bin|rpm)";
...
files=`ls $PWD/ibm-semeru*{.tar.gz,.sha256.txt,.zip,.pkg,.msi,.json,.bin,.rpm} | sed -e ':a' -e 'N' -e '$!ba' -e 's/\n/ /g'`
